#!/usr/bin/env python3
"""
SAM2 Bidirectional Video Segmentation Pipeline
Processes videos in both forward (from annotated frame) and backward (to annotated frame) directions.
Generates cropped per-object videos without mask overlay.
"""

import os
import sys
import json
import argparse
import cv2
import numpy as np
import torch
import matplotlib.pyplot as plt
from PIL import Image
from datetime import datetime
from pathlib import Path

from sam2.build_sam import build_sam2_video_predictor


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(
        description='SAM2 Bidirectional Video Segmentation Pipeline',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    
    parser.add_argument(
        '--input-folder',
        type=str,
        required=True,
        help='Input folder containing video file and annotation JSON'
    )
    
    parser.add_argument(
        '--output-folder',
        type=str,
        default=None,
        help='Output folder for generated videos'
    )
    
    parser.add_argument(
        '--processed-folder',
        type=str,
        default=None,
        help='Processed folder for intermediate files'
    )
    
    parser.add_argument(
        '--model-config',
        type=str,
        default='configs/sam2.1/sam2.1_hiera_t.yaml',
        help='SAM2 model configuration file'
    )
    
    parser.add_argument(
        '--checkpoint',
        type=str,
        default='checkpoints/sam2.1_hiera_tiny.pt',
        help='SAM2 model checkpoint file'
    )
    
    parser.add_argument(
        '--fps',
        type=int,
        default=20,
        help='Target FPS for frame extraction and output videos'
    )
    
    parser.add_argument(
        '--device',
        type=str,
        default='cuda',
        choices=['cuda', 'cpu', 'mps'],
        help='Device to run inference on'
    )
    
    return parser.parse_args()


def setup_device(device_name):
    """Setup computation device."""
    if device_name == 'cuda' and torch.cuda.is_available():
        device = torch.device("cuda")
        torch.autocast("cuda", dtype=torch.bfloat16).__enter__()
        if torch.cuda.get_device_properties(0).major >= 8:
            torch.backends.cuda.matmul.allow_tf32 = True
            torch.backends.cudnn.allow_tf32 = True
        print(f"Using device: CUDA ({torch.cuda.get_device_name(0)})")
    elif device_name == 'mps' and torch.backends.mps.is_available():
        device = torch.device("mps")
        print("Using device: MPS (Apple Silicon)")
    else:
        device = torch.device("cpu")
        print("Using device: CPU")
    
    return device


def find_input_files(input_folder):
    """Find video and JSON annotation files in input folder."""
    json_files = [f for f in os.listdir(input_folder) if f.endswith('.json')]
    if len(json_files) == 0:
        raise ValueError(f"No JSON annotation file found in {input_folder}")
    elif len(json_files) > 1:
        print(f"⚠ Multiple JSON files found, using first: {json_files[0]}")
    
    annotation_file = os.path.join(input_folder, json_files[0])
    
    video_extensions = ['.mp4', '.avi', '.mov', '.mkv']
    video_files = [f for f in os.listdir(input_folder) 
                   if any(f.endswith(ext) for ext in video_extensions)]
    
    if len(video_files) == 0:
        raise ValueError(f"No video file found in {input_folder}")
    elif len(video_files) > 1:
        print(f"⚠ Multiple video files found, using first: {video_files[0]}")
    
    video_path = os.path.join(input_folder, video_files[0])
    
    print(f"✓ Found annotation: {os.path.basename(annotation_file)}")
    print(f"✓ Found video: {os.path.basename(video_path)}")
    
    return video_path, annotation_file


def preprocess_video_bidirectional(video_path, annotation_file, processed_dir):
    """
    Preprocess video in both forward and backward directions.
    Returns: (forward_props, backward_props) with all necessary info
    """
    print(f"\n{'='*70}")
    print(f"STEP 1: BIDIRECTIONAL PREPROCESSING")
    print(f"{'='*70}")
    
    # Load JSON
    with open(annotation_file, 'r') as f:
        data = json.load(f)
    
    # Find first annotated frame
    first_annotated_frame_no = None
    num_boxes = 0
    
    for frame_data in data["frames"]:
        boxes = frame_data.get("annotation", {}).get("boxes", [])
        if len(boxes) > 0:
            first_annotated_frame_no = frame_data["frameNo"]
            num_boxes = len(boxes)
            print(f"✓ Found annotated frame: frameNo {first_annotated_frame_no}")
            print(f"  Number of objects: {num_boxes}")
            break
    
    if first_annotated_frame_no is None:
        raise ValueError("No annotated frames found in JSON!")
    
    # Load video properties
    cap = cv2.VideoCapture(video_path)
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    fps = cap.get(cv2.CAP_PROP_FPS)
    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    cap.release()
    
    print(f"\n📹 Original video properties:")
    print(f"  Total frames: {total_frames}")
    print(f"  FPS: {fps}")
    print(f"  Resolution: {width}x{height}")
    print(f"  Annotated frame (1-indexed): {first_annotated_frame_no}")
    
    video_name_base = os.path.splitext(os.path.basename(video_path))[0]
    
    # ==================== FORWARD DIRECTION ====================
    print(f"\n{'─'*70}")
    print(f"FORWARD: From frame {first_annotated_frame_no} onwards")
    print(f"{'─'*70}")
    
    forward_dir = os.path.join(processed_dir, "preprocessed_forward")
    os.makedirs(forward_dir, exist_ok=True)
    
    trim_start_frame = first_annotated_frame_no - 1  # Convert to 0-indexed
    frames_to_extract = total_frames - trim_start_frame
    
    trimmed_video_path_fwd = os.path.join(forward_dir, f"{video_name_base}_forward.mp4")
    
    print(f"Trimming video from frame {trim_start_frame}...")
    cap = cv2.VideoCapture(video_path)
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out_writer = cv2.VideoWriter(trimmed_video_path_fwd, fourcc, fps, (width, height))
    
    cap.set(cv2.CAP_PROP_POS_FRAMES, trim_start_frame)
    fwd_frames_written = 0
    
    while True:
        ret, frame = cap.read()
        if not ret:
            break
        out_writer.write(frame)
        fwd_frames_written += 1
    
    cap.release()
    out_writer.release()
    
    print(f"✓ Forward video: {fwd_frames_written} frames")
    
    # Create forward JSON
    processed_frames_fwd = []
    frame_offset = first_annotated_frame_no - 1
    
    for frame_data in data["frames"]:
        original_frame_no = frame_data["frameNo"]
        if original_frame_no >= first_annotated_frame_no:
            new_frame_no = original_frame_no - frame_offset
            new_frame_data = frame_data.copy()
            new_frame_data["frameNo"] = new_frame_no
            new_frame_data["originalFrameNo"] = original_frame_no
            processed_frames_fwd.append(new_frame_data)
    
    processed_json_fwd = {
        "taskId": data.get("taskId", ""),
        "campaignId": data.get("campaignId", ""),
        "videoName": f"{video_name_base}_forward",
        "metadata": {
            "fps": fps,
            "totalFrames": fwd_frames_written,
            "direction": "forward",
            "originalAnnotatedFrame": first_annotated_frame_no,
            "processedDate": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        },
        "frames": processed_frames_fwd
    }
    
    processed_json_path_fwd = os.path.join(forward_dir, f"{video_name_base}_forward.json")
    with open(processed_json_path_fwd, 'w') as f:
        json.dump(processed_json_fwd, f, indent=2)
    
    print(f"✓ Forward JSON: {len(processed_frames_fwd)} frames")
    
    # ==================== BACKWARD DIRECTION ====================
    print(f"\n{'─'*70}")
    print(f"BACKWARD: From frame {first_annotated_frame_no} to frame 1")
    print(f"{'─'*70}")
    
    backward_dir = os.path.join(processed_dir, "preprocessed_backward")
    os.makedirs(backward_dir, exist_ok=True)
    
    # Extract frames 0 to first_annotated_frame_no-1 in reverse
    bwd_frame_indices = list(range(0, first_annotated_frame_no))
    bwd_frame_indices.reverse()  # Now in reverse order
    
    trimmed_video_path_bwd = os.path.join(backward_dir, f"{video_name_base}_backward.mp4")
    
    print(f"Extracting and reversing frames 0-{first_annotated_frame_no-1}...")
    cap = cv2.VideoCapture(video_path)
    out_writer = cv2.VideoWriter(trimmed_video_path_bwd, fourcc, fps, (width, height))
    
    bwd_frames_written = 0
    for idx in bwd_frame_indices:
        cap.set(cv2.CAP_PROP_POS_FRAMES, idx)
        ret, frame = cap.read()
        if ret:
            out_writer.write(frame)
            bwd_frames_written += 1
    
    cap.release()
    out_writer.release()
    
    print(f"✓ Backward video: {bwd_frames_written} frames (reversed)")
    
    # Create backward JSON (frames renumbered, reversed)
    processed_frames_bwd = []
    
    # Get all frames UP TO AND INCLUDING annotated frame
    frames_up_to_annotated = [f for f in data["frames"] 
                              if f["frameNo"] <= first_annotated_frame_no]
    
    # Reverse them
    frames_up_to_annotated.reverse()
    
    for new_idx, frame_data in enumerate(frames_up_to_annotated):
        new_frame_data = frame_data.copy()
        new_frame_data["frameNo"] = new_idx + 1  # Renumber from 1
        new_frame_data["originalFrameNo"] = frame_data["frameNo"]
        processed_frames_bwd.append(new_frame_data)
    
    processed_json_bwd = {
        "taskId": data.get("taskId", ""),
        "campaignId": data.get("campaignId", ""),
        "videoName": f"{video_name_base}_backward",
        "metadata": {
            "fps": fps,
            "totalFrames": bwd_frames_written,
            "direction": "backward",
            "originalAnnotatedFrame": first_annotated_frame_no,
            "processedDate": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        },
        "frames": processed_frames_bwd
    }
    
    processed_json_path_bwd = os.path.join(backward_dir, f"{video_name_base}_backward.json")
    with open(processed_json_path_bwd, 'w') as f:
        json.dump(processed_json_bwd, f, indent=2)
    
    print(f"✓ Backward JSON: {len(processed_frames_bwd)} frames (reversed)")
    
    print(f"\n{'='*70}")
    print(f"✅ BIDIRECTIONAL PREPROCESSING COMPLETE")
    print(f"{'='*70}\n")
    
    video_properties = {
        'width': width,
        'height': height,
        'fps': fps,
        'num_boxes': num_boxes
    }
    
    forward_props = {
        'video_path': trimmed_video_path_fwd,
        'json_path': processed_json_path_fwd,
        'frames_count': fwd_frames_written,
        'direction': 'forward'
    }
    
    backward_props = {
        'video_path': trimmed_video_path_bwd,
        'json_path': processed_json_path_bwd,
        'frames_count': bwd_frames_written,
        'direction': 'backward'
    }
    
    return forward_props, backward_props, video_properties


def extract_frames(video_path, output_dir, direction, target_fps=20):
    """Extract frames from video at target FPS."""
    print(f"\nExtracting frames for {direction}...")
    
    frames_dir = os.path.join(output_dir, "frames")
    os.makedirs(frames_dir, exist_ok=True)
    
    cap = cv2.VideoCapture(video_path)
    original_fps = cap.get(cv2.CAP_PROP_FPS)
    
    frame_interval = int(original_fps / target_fps) if original_fps > target_fps else 1
    
    idx = 0
    frame_count = 0
    
    while True:
        ret, frame = cap.read()
        if not ret:
            break
        
        if idx % frame_interval == 0:
            cv2.imwrite(f"{frames_dir}/{frame_count:05d}.jpg", frame)
            frame_count += 1
        
        idx += 1
    
    cap.release()
    
    print(f"  {direction}: {frame_count} frames extracted")
    
    return frames_dir


def load_frames(frames_dir, annotation_file):
    """Load frame names and find first annotated frame."""
    frame_names = [p for p in os.listdir(frames_dir)
                   if os.path.splitext(p)[-1].lower() in [".jpg", ".jpeg"]]
    frame_names.sort(key=lambda p: int(os.path.splitext(p)[0]))
    
    with open(annotation_file, 'r') as f:
        data = json.load(f)
    
    START_FRAME = None
    num_boxes = 0
    
    for frame_data in data["frames"]:
        boxes = frame_data.get("annotation", {}).get("boxes", [])
        if len(boxes) > 0:
            START_FRAME = frame_data["frameNo"] - 1  # Convert to 0-indexed
            num_boxes = len(boxes)
            break
    
    if START_FRAME is None:
        # No annotated frames in this direction
        return frame_names, data, None
    
    return frame_names, data, START_FRAME


def initialize_objects(predictor, inference_state, data, START_FRAME, width, height):
    """Initialize SAM2 with bounding boxes from annotations."""
    if START_FRAME is None:
        return {}
    
    annotated_boxes = []
    for frame_data in data["frames"]:
        if frame_data["frameNo"] - 1 == START_FRAME:
            annotated_boxes = frame_data["annotation"]["boxes"]
            break
    
    all_masks = {}
    
    for idx, box_data in enumerate(annotated_boxes):
        coords = box_data["coordinates"]
        
        x_coords = [coords[f"point{i}"]["x"] for i in range(1, 5)]
        y_coords = [coords[f"point{i}"]["y"] for i in range(1, 5)]
        
        x_min = int(min(x_coords) * width)
        y_min = int(min(y_coords) * height)
        x_max = int(max(x_coords) * width)
        y_max = int(max(y_coords) * height)
        
        box = np.array([x_min, y_min, x_max, y_max], dtype=np.float32)
        
        obj_id = idx + 1
        label = str(box_data.get("label", f"object_{obj_id}"))
        
        _, out_obj_ids, out_mask_logits = predictor.add_new_points_or_box(
            inference_state=inference_state,
            frame_idx=START_FRAME,
            obj_id=obj_id,
            box=box,
        )
        
        mask_idx = list(out_obj_ids).index(obj_id)
        
        all_masks[obj_id] = {
            'mask_logits': out_mask_logits[mask_idx],
            'mask': (out_mask_logits[mask_idx] > 0.0).cpu().numpy(),
            'box': box,
            'label': label
        }
    
    return all_masks


def process_direction(model_cfg, checkpoint, device, direction_props, 
                     processed_dir, output_base_dir, video_name_base, video_properties, fps):
    """
    Process one direction (forward or backward).
    Returns: (success, all_masks, frames_dir, frame_names, data, predictor, inference_state)
    """
    direction = direction_props['direction']
    print(f"\n{'='*70}")
    print(f"PROCESSING {direction.upper()} DIRECTION")
    print(f"{'='*70}")
    
    # Extract frames
    direction_processed_dir = os.path.join(processed_dir, f"preprocessed_{direction}")
    frames_dir = extract_frames(direction_props['video_path'], direction_processed_dir, direction, fps)
    
    # Load frames
    frame_names, data, START_FRAME = load_frames(frames_dir, direction_props['json_path'])
    
    print(f"✓ Loaded {len(frame_names)} frames")
    
    # If no annotated frame in this direction, return raw frames
    if START_FRAME is None:
        print(f"ℹ No annotated frames in {direction} direction, will use raw frames")
        return True, {}, frames_dir, frame_names, data, None, None
    
    # Try SAM2 inference
    try:
        print(f"\n🔧 Initializing SAM2 for {direction}...")
        predictor = build_sam2_video_predictor(model_cfg, checkpoint, device=device)
        inference_state = predictor.init_state(video_path=frames_dir)
        
        print(f"🔧 Initializing objects...")
        all_masks = initialize_objects(predictor, inference_state, data, START_FRAME, 
                                      video_properties['width'], video_properties['height'])
        
        print(f"✓ SAM2 ready for {direction}")
        return True, all_masks, frames_dir, frame_names, data, predictor, inference_state
        
    except Exception as e:
        print(f"\n❌ SAM2 inference failed for {direction}: {str(e)}")
        print(f"⚠ Will use raw frames for {direction}")
        return False, {}, frames_dir, frame_names, data, None, None


def create_videos(all_masks_fwd, all_masks_bwd, frames_dir_fwd, frames_dir_bwd, 
                 frame_names_fwd, frame_names_bwd, data_fwd, data_bwd,
                 output_dir, video_name_base, width, height, fps, video_properties,
                 predictor_fwd, inference_state_fwd, predictor_bwd, inference_state_bwd):
    """
    Create all output videos:
    - 4 main overlay videos (combined forward+backward)
    - Per-object cropped videos (no mask overlay)
    """
    print(f"\n{'='*70}")
    print(f"CREATING OUTPUT VIDEOS")
    print(f"{'='*70}")
    
    # ==================== FORWARD PROPAGATION ====================
    forward_masks_by_frame = {}
    
    if all_masks_fwd:
        print(f"\nPropagating masks (FORWARD)...")
        for out_frame_idx, out_obj_ids, out_mask_logits in predictor_fwd.propagate_in_video(inference_state_fwd):
            forward_masks_by_frame[out_frame_idx] = {}
            for i, out_obj_id in enumerate(out_obj_ids):
                mask = (out_mask_logits[i] > 0.0).cpu().numpy().squeeze()
                forward_masks_by_frame[out_frame_idx][out_obj_id] = mask
    
    print(f"  Forward: {len(forward_masks_by_frame)} frames with masks")
    
    # ==================== BACKWARD PROPAGATION ====================
    backward_masks_by_frame = {}
    
    if all_masks_bwd:
        print(f"\nPropagating masks (BACKWARD)...")
        for out_frame_idx, out_obj_ids, out_mask_logits in predictor_bwd.propagate_in_video(inference_state_bwd):
            backward_masks_by_frame[out_frame_idx] = {}
            for i, out_obj_id in enumerate(out_obj_ids):
                mask = (out_mask_logits[i] > 0.0).cpu().numpy().squeeze()
                backward_masks_by_frame[out_frame_idx][out_obj_id] = mask
    
    print(f"  Backward: {len(backward_masks_by_frame)} frames with masks (before re-reversing)")
    
    # RE-REVERSE backward masks to chronological order
    # backward_masks_by_frame has indices [0, 1, 2, ...] corresponding to reversed frames
    # We need to map them back to original chronological positions
    backward_masks_by_frame_reversed = {}
    
    if backward_masks_by_frame:
        # Total frames in backward direction
        total_bwd_frames = len(frame_names_bwd)
        for reversed_idx, masks_dict in backward_masks_by_frame.items():
            # Convert reversed index back to chronological index
            chrono_idx = total_bwd_frames - 1 - reversed_idx
            backward_masks_by_frame_reversed[chrono_idx] = masks_dict
    
    print(f"  Backward: {len(backward_masks_by_frame_reversed)} frames re-reversed to chronological order")
    
    # ==================== CREATE OVERLAY VIDEOS ====================
    os.makedirs(output_dir, exist_ok=True)
    
    overlay_both_path = os.path.join(output_dir, f"{video_name_base}_combined_masks_and_boxes.mp4")
    overlay_boxes_path = os.path.join(output_dir, f"{video_name_base}_combined_boxes.mp4")
    overlay_masks_blended_path = os.path.join(output_dir, f"{video_name_base}_combined_masks_overlaid.mp4")
    overlay_masks_only_path = os.path.join(output_dir, f"{video_name_base}_combined_masks_only.mp4")
    
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    
    writer_both = cv2.VideoWriter(overlay_both_path, fourcc, fps, (width, height))
    writer_boxes = cv2.VideoWriter(overlay_boxes_path, fourcc, fps, (width, height))
    writer_masks_blended = cv2.VideoWriter(overlay_masks_blended_path, fourcc, fps, (width, height))
    writer_masks_only = cv2.VideoWriter(overlay_masks_only_path, fourcc, fps, (width, height))
    
    # ==================== CREATE PER-OBJECT CROPPED VIDEOS ====================
    objects_dir = os.path.join(output_dir, "objects_cropped")
    os.makedirs(objects_dir, exist_ok=True)
    
    # Collect all unique objects
    all_objects = {}
    for masks_dict in [all_masks_fwd, all_masks_bwd]:
        for obj_id, mask_info in masks_dict.items():
            if obj_id not in all_objects:
                all_objects[obj_id] = mask_info
    
    object_writers = {}
    
    for obj_id, mask_info in all_objects.items():
        label = mask_info.get('label', f'obj_{obj_id}')
        clean_label = ''.join(c if c.isalnum() or c in ['-', '_'] else '_' for c in str(label))
        
        # Get crop region from bounding box
        box = mask_info['box']
        x1, y1, x2, y2 = int(box[0]), int(box[1]), int(box[2]), int(box[3])
        crop_width = x2 - x1
        crop_height = y2 - y1
        
        obj_video_path = os.path.join(objects_dir, f"{video_name_base}_object_{clean_label}_cropped.mp4")
        obj_writer = cv2.VideoWriter(obj_video_path, fourcc, fps, (crop_width, crop_height))
        
        if obj_writer.isOpened():
            object_writers[obj_id] = {
                'writer': obj_writer,
                'box': box,
                'label': label,
                'path': obj_video_path
            }
    
    print(f"\n📹 Output videos:")
    print(f"  4 main overlay videos (combined forward+backward)")
    print(f"  {len(object_writers)} cropped per-object videos (no mask overlay)")
    
    # ==================== WRITE FRAMES ====================
    cmap = plt.get_cmap("tab10")
    
    frame_count_total = 0
    
    # Write backward frames (in chronological order now)
    print(f"\nWriting BACKWARD frames (chronological order after re-reversing)...")
    for frame_idx in range(len(frame_names_bwd)):
        frame_path = os.path.join(frames_dir_bwd, frame_names_bwd[frame_idx])
        frame = cv2.imread(frame_path)
        
        if frame is None:
            continue
        
        overlay_both = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        overlay_boxes = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        overlay_masks_blended = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        overlay_masks_only = np.zeros((height, width, 3), dtype=np.uint8)
        
        # Draw masks (using re-reversed index mapping)
        if frame_idx in backward_masks_by_frame_reversed:
            for obj_id, mask in backward_masks_by_frame_reversed[frame_idx].items():
                color = np.array([*cmap(obj_id)[:3]])
                mask_3channel = np.repeat(mask[:, :, np.newaxis], 3, axis=2)
                color_rgb = color.reshape(1, 1, 3)
                mask_colored = (mask_3channel * color_rgb * 255).astype(np.uint8)
                
                overlay_both = cv2.addWeighted(overlay_both, 1.0, mask_colored, 0.6, 0)
                overlay_masks_blended = cv2.addWeighted(overlay_masks_blended, 1.0, mask_colored, 0.6, 0)
                overlay_masks_only = cv2.add(overlay_masks_only, mask_colored)
        
        # Draw boxes
        for obj_id, mask_info in all_masks_bwd.items():
            box = mask_info['box']
            x1, y1, x2, y2 = int(box[0]), int(box[1]), int(box[2]), int(box[3])
            label = mask_info.get('label', f'obj_{obj_id}')
            
            cv2.rectangle(overlay_both, (x1, y1), (x2, y2), (0, 255, 0), 2)
            cv2.putText(overlay_both, str(label), (x1, y1-5), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            
            cv2.rectangle(overlay_boxes, (x1, y1), (x2, y2), (0, 255, 0), 2)
            cv2.putText(overlay_boxes, str(label), (x1, y1-5), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        
        # Write overlay videos
        writer_both.write(cv2.cvtColor(overlay_both, cv2.COLOR_RGB2BGR))
        writer_boxes.write(cv2.cvtColor(overlay_boxes, cv2.COLOR_RGB2BGR))
        writer_masks_blended.write(cv2.cvtColor(overlay_masks_blended, cv2.COLOR_RGB2BGR))
        writer_masks_only.write(cv2.cvtColor(overlay_masks_only, cv2.COLOR_RGB2BGR))
        
        # Write cropped per-object videos
        for obj_id, obj_info in object_writers.items():
            obj_writer = obj_info['writer']
            box = obj_info['box']
            x1, y1, x2, y2 = int(box[0]), int(box[1]), int(box[2]), int(box[3])
            
            # Crop the original frame (no mask overlay)
            cropped_frame = frame[y1:y2, x1:x2]
            
            if cropped_frame.size > 0:
                obj_writer.write(cropped_frame)
        
        frame_count_total += 1
        if frame_count_total % 20 == 0:
            print(f"  Progress: {frame_count_total}/{len(frame_names_bwd)} (backward)", end='\r')
    
    # Write forward frames (SKIP first frame to avoid duplication with backward)
    print(f"\nWriting FORWARD frames (skipping annotated frame already in backward)...")
    for frame_idx in range(1, len(frame_names_fwd)):
        frame_path = os.path.join(frames_dir_fwd, frame_names_fwd[frame_idx])
        frame = cv2.imread(frame_path)
        
        if frame is None:
            continue
        
        overlay_both = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        overlay_boxes = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        overlay_masks_blended = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        overlay_masks_only = np.zeros((height, width, 3), dtype=np.uint8)
        
        # Draw masks
        if frame_idx in forward_masks_by_frame:
            for obj_id, mask in forward_masks_by_frame[frame_idx].items():
                color = np.array([*cmap(obj_id)[:3]])
                mask_3channel = np.repeat(mask[:, :, np.newaxis], 3, axis=2)
                color_rgb = color.reshape(1, 1, 3)
                mask_colored = (mask_3channel * color_rgb * 255).astype(np.uint8)
                
                overlay_both = cv2.addWeighted(overlay_both, 1.0, mask_colored, 0.6, 0)
                overlay_masks_blended = cv2.addWeighted(overlay_masks_blended, 1.0, mask_colored, 0.6, 0)
                overlay_masks_only = cv2.add(overlay_masks_only, mask_colored)
        
        # Draw boxes
        for obj_id, mask_info in all_masks_fwd.items():
            box = mask_info['box']
            x1, y1, x2, y2 = int(box[0]), int(box[1]), int(box[2]), int(box[3])
            label = mask_info.get('label', f'obj_{obj_id}')
            
            cv2.rectangle(overlay_both, (x1, y1), (x2, y2), (0, 255, 0), 2)
            cv2.putText(overlay_both, str(label), (x1, y1-5), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            
            cv2.rectangle(overlay_boxes, (x1, y1), (x2, y2), (0, 255, 0), 2)
            cv2.putText(overlay_boxes, str(label), (x1, y1-5), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        
        # Write overlay videos
        writer_both.write(cv2.cvtColor(overlay_both, cv2.COLOR_RGB2BGR))
        writer_boxes.write(cv2.cvtColor(overlay_boxes, cv2.COLOR_RGB2BGR))
        writer_masks_blended.write(cv2.cvtColor(overlay_masks_blended, cv2.COLOR_RGB2BGR))
        writer_masks_only.write(cv2.cvtColor(overlay_masks_only, cv2.COLOR_RGB2BGR))
        
        # Write cropped per-object videos
        for obj_id, obj_info in object_writers.items():
            obj_writer = obj_info['writer']
            box = obj_info['box']
            x1, y1, x2, y2 = int(box[0]), int(box[1]), int(box[2]), int(box[3])
            
            # Crop the original frame (no mask overlay)
            cropped_frame = frame[y1:y2, x1:x2]
            
            if cropped_frame.size > 0:
                obj_writer.write(cropped_frame)
        
        frame_count_total += 1
        if frame_count_total % 20 == 0:
            print(f"  Progress: {frame_count_total}/{len(frame_names_fwd)-1} (forward)", end='\r')
    
    # Release writers
    writer_both.release()
    writer_boxes.release()
    writer_masks_blended.release()
    writer_masks_only.release()
    
    for obj_info in object_writers.values():
        obj_info['writer'].release()
    
    # ==================== SUMMARY ====================
    print(f"\n\n{'='*70}")
    print(f"✅ VIDEO GENERATION COMPLETE")
    print(f"{'='*70}")
    
    print(f"\n📹 Main Overlay Videos (4 total - combined forward+backward):")
    videos_to_check = [
        ("Masks + Boxes", overlay_both_path),
        ("Boxes Only", overlay_boxes_path),
        ("Masks Overlaid", overlay_masks_blended_path),
        ("Masks Only (solid)", overlay_masks_only_path)
    ]
    
    for vid_name, vid_path in videos_to_check:
        exists = os.path.exists(vid_path)
        size = os.path.getsize(vid_path) / 1024 / 1024 if exists else 0
        status = '✓' if exists else '✗'
        print(f"  {status} {vid_name}: {size:.2f} MB")
    
    print(f"\n📁 objects_cropped/ folder:")
    print(f"  {len(object_writers)} cropped per-object videos (no mask overlay)")
    for obj_id, obj_info in object_writers.items():
        exists = os.path.exists(obj_info['path'])
        size = os.path.getsize(obj_info['path']) / 1024 / 1024 if exists else 0
        status = '✓' if exists else '✗'
        print(f"    {status} {obj_info['label']}: {size:.2f} MB")
    
    total_videos = 4 + len(object_writers)
    print(f"\n📊 Total videos: {total_videos}")
    print(f"   - 4 main overlay videos")
    print(f"   - {len(object_writers)} cropped object videos")
    
    print(f"\n{'='*70}")
    print(f"💡 All videos saved to: {output_dir}")
    print(f"{'='*70}\n")


def main():
    """Main pipeline execution."""
    args = parse_args()
    
    input_folder = os.path.abspath(args.input_folder)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    input_folder_name = os.path.basename(input_folder.rstrip('/'))
    
    if args.processed_folder:
        processed_folder = os.path.abspath(args.processed_folder)
    else:
        processed_folder = os.path.join(script_dir, "processed_bidirectional", input_folder_name)
    
    if args.output_folder:
        output_folder = os.path.abspath(args.output_folder)
    else:
        output_folder = os.path.join(script_dir, "output_bidirectional", input_folder_name)
    
    print(f"\n{'='*70}")
    print(f"SAM2 BIDIRECTIONAL VIDEO SEGMENTATION PIPELINE")
    print(f"{'='*70}")
    print(f"Input folder: {input_folder}")
    print(f"Processed folder: {processed_folder}")
    print(f"Output folder: {output_folder}")
    print(f"{'='*70}\n")
    
    # Setup device
    device = setup_device(args.device)
    
    # Find input files
    video_path, annotation_file = find_input_files(input_folder)
    
    # Step 1: Bidirectional preprocessing
    forward_props, backward_props, video_properties = preprocess_video_bidirectional(
        video_path, annotation_file, processed_folder
    )
    
    # Step 2: Process forward direction
    success_fwd, all_masks_fwd, frames_dir_fwd, frame_names_fwd, data_fwd, predictor_fwd, inference_state_fwd = process_direction(
        args.model_config, args.checkpoint, device, forward_props, processed_folder,
        output_folder, os.path.splitext(os.path.basename(video_path))[0], video_properties, args.fps
    )
    
    # Step 3: Process backward direction
    success_bwd, all_masks_bwd, frames_dir_bwd, frame_names_bwd, data_bwd, predictor_bwd, inference_state_bwd = process_direction(
        args.model_config, args.checkpoint, device, backward_props, processed_folder,
        output_folder, os.path.splitext(os.path.basename(video_path))[0], video_properties, args.fps
    )
    
    # Step 4: Create output videos
    create_videos(
        all_masks_fwd if predictor_fwd else {}, all_masks_bwd if predictor_bwd else {},
        frames_dir_fwd, frames_dir_bwd, frame_names_fwd, frame_names_bwd,
        data_fwd, data_bwd, output_folder, 
        os.path.splitext(os.path.basename(video_path))[0],
        video_properties['width'], video_properties['height'], args.fps, video_properties,
        predictor_fwd, inference_state_fwd, predictor_bwd, inference_state_bwd
    )
    
    print("\n🎉 Bidirectional pipeline completed successfully!")


if __name__ == "__main__":
    main()
